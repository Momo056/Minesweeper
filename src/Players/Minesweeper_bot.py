...
class Minesweeper_bot(Player_Interface):
    ...
    def action(self, grid: ndarray, grid_view: ndarray) -> tuple[int, int]:
        ...
        if np.all(~self.grid_view):
            ...
            if self.random_first_move:
                return randint(0, grid_view.shape[0]-1), randint(0, grid_view.shape[1]-1)
        ...
        self.to_cross_inspect = list(np.argwhere(np.logical_and(
            grid_view,
            grid > 0
        ))) # TODO : Supress boxs without neighbours
        self.to_cross_inspect = [(p1, p2) for p1, p2 in product(self.to_cross_inspect, self.to_cross_inspect) if not np.all(p1 == p2)]
    ...
    def cross_inspect(self, p1: tuple[float, float], p2: tuple[float, float]) -> tuple[int, int] | None:
