...
class Game_Tensor_Interface:
    ...
    def to_tensor(self, grid: ndarray, grid_view: ndarray) -> Tensor:
        ...
        return torch.permute(
            one_hot(t, self.num_classes).type(torch.float32), (-1, 0, 1)
        )
    ...
    def visible_grid(tensor_representation: Tensor):
        ...
        grid_values = torch.sum(
            tensor_representation[:9]
            * torch.arange(9, device=tensor_representation.device).view((9, 1, 1)),
            dim=0,
        ).type(torch.uint8)
        return np.array(grid_values).astype(np.uint8), ~(
            np.array(grid_view).astype(np.bool_)
        )

    def view_grid_tensor(
        tensor_representation: Tensor,
        mines: Tensor | None = None,
        view_grid_kwargs: dict[str, Any] = {},
    ) -> None:
        return view_grid(
            *Game_Tensor_Interface.visible_grid(tensor_representation),
            mines=None if mines is None else np.array(mines),
            **view_grid_kwargs
        )
