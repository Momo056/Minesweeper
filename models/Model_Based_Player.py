...
class Model_Based_Player(Player_Interface):
    def __init__(
        self, model: Module, model_deivce="cuda", out_of_boundary_treshold: float = 0.1
    ) -> None:
        super().__init__()
    ...
    def action(self, grid: ndarray, grid_view: ndarray) -> tuple[int, int]:
        tensor_representation = (
            self.tensor_interface.to_tensor(grid, grid_view)
            .type(torch.float32)
            .to(self.model_deivce)
        )
        self.model.eval()
        with torch.no_grad():
        ...
        boundary = Game_Tensor_Interface.unknown_boundaries(
            tensor_representation.reshape(1, *tensor_representation.shape)
        )[0]
